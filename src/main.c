...
PVOID sendGstreamerAudioVideo(PVOID args)
{
  ...
  if ((GETENV(PIPELINE)) != NULL) {

        pipeline=gst_parse_launch(GETENV(PIPELINE),
                                &error);
   }
   
  else {

        pipeline = gst_parse_launch("v4l2src do-timestamp=TRUE device=/dev/video0 ! "
                                "queue ! "
                                "videoconvert ! "
...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    CHK_ERR((pChannelName = getenv(IOT_CORE_THING_NAME)) != NULL, STATUS_INVALID_OPERATION, "IOT_CORE_THING_NAME must be set");
#else
    pChannelName = argc > 1 ? argv[1] : SAMPLE_CHANNEL_NAME;
#endif
